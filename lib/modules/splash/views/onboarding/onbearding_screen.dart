...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  final List<Map<String, String>> _onboardingData = [
    {"title": "مرحباً بك في تطبيقنا!", "description": "اكتشف ميزات رائعة تجعل حياتك أسهل.", "image": AppAssetsImages.onboarding.onboarding1},
    {"title": "سهولة الاستخدام", "description": "واجهة بسيطة وتصميم جذاب لتجربة مستخدم ممتعة.", "image": AppAssetsImages.onboarding.onboarding2},
    {"title": "ابدأ الآن!", "description": "انضم إلينا واستمتع بكل ما نقدمه لك.", "image": AppAssetsImages.onboarding.onboarding3},
  ];
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          actions: [TextButton(onPressed: _onSkip, child: const Text("تخطي"))],
        ),
        body: Column(
          children: [
            Expanded(
            ...
            // مؤشر الصفحات (النقاط)
            Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(_onboardingData.length, (index) => buildDot(index, context))),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (_currentPage > 0)
                    OutlinedButton(onPressed: _onBack, child: const Text("السابق"))
                  else
                    // عنصر نائب للحفاظ على التباعد عندما يكون زر الرجوع مخفياً
                    const SizedBox(width: 100), // يمكنك تعديل العرض حسب الحاجة
                  // زر التالي أو إنهاء
                  ElevatedButton(onPressed: _onNext, child: Text(_currentPage == _onboardingData.length - 1 ? "إنهاء" : "التالي")),
                ],
              ),
            ),
          ],
...
class OnboardingPageContent extends StatelessWidget {
  ...
  const OnboardingPageContent({super.key, required this.title, required this.description, required this.imagePath});
